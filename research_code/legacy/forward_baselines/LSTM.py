...
class CNN_LSTM(nn.Module):
    def __init__(self,n_filter,conv_input_channel,conv_output_channel, hidden_size, num_layers, output_size,dropout,training):
        ...
        self.fc = nn.Sequential(nn.Linear(in_features=9,out_features=64),nn.ReLU(),
                                nn.Linear(in_features=64,out_features=512),nn.ReLU(),
                                nn.Linear(in_features=512,out_features=64),nn.Tanh(),
...
def train2(train_dataloader,test_dataloader,epochs,
    ...
    for epoch in range(1,epochs+1):
        ...
        for idx,(x,y) in enumerate(train_dataloader,1):
            data_x = x.to(torch.float32)
            data_y = y.to(torch.float32)
...
def test():
    ...
    x_train_scaler,x_test_scaler,x_val_scaler = min_max(x_train,x_test,x_val)
    y_train_scaler, y_test_scaler, y_val_scaler = min_max(y_train, y_test, y_val)
