...
class CNN_LSTM(nn.Module):
    def __init__(self,n_filter,conv_input_channel,conv_output_channel, hidden_size, num_layers, output_size,dropout,training):
        ...
        self.convs = nn.ModuleList([nn.Conv1d(conv_input_channel, conv_output_channel, kernel_size = filter_size, bias=True)
                                    for filter_size in self.filter_sizes])


        #lstm+linear
        self.lstm = nn.LSTM(self.conv_output_channel*len(self.filter_sizes), hidden_size, num_layers, batch_first = True)
...
def train2(x_train_scaler,y_train_scaler,x_test_scaler,y_test_scaler
    ...
    for epoch in range(1,epochs+1):
        ...
        valid_epoch_loss = []
        with torch.no_grad():
            for idx,batch in enumerate(data_iter(x_test_scaler,y_test_scaler, batch_size = batch_size, shuffle=True),1):
                data_x = batch[:,0:-1].unsqueeze(1).to(torch.float32)
                data_y = batch[:,-1].to(torch.float32)
                outputs = model(data_x)
                loss = criterion(data_y, outputs)
...
def test():
    data = np.loadtxt('./稀疏采样_300.txt',delimiter='\t')
    x = data[:,0:9]
    y =data[:,9:11]
