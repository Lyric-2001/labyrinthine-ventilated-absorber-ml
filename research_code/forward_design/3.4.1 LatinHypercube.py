...
def test():
    ...
    print(data)
    np.savetxt('./LHS_train_sample.txt',data,delimiter='\t')
    #np.savetxt('./LHS_test_comsol_sample.txt',data,fmt='%.1f')
#查看所抽样的数据
...
if __name__ == '__main__':
    test()
    a = np.loadtxt('./LHS_train_sample.txt')

    plt.figure(figsize=(12, 5))
    plt.subplot(1, 2, 1)
    plt.scatter(a[:, 0], a[:, 1])
    plt.title("(a)", fontdict={'fontsize': 16})
    ...
    plt.subplot(1, 2,2)
    plt.scatter(a[:, 3], a[:, 4])
    plt.title("(b)", fontdict={'fontsize': 16})
